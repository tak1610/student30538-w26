...
* Two ways to 'save' across re-runs of your dashboard
    * Session state
    * Caching


## Why dashboards are different than `.py` 
...
* "Streamlit mode" (`$ streamlit run app.py`) does four things:
    ...
    * reruns code when it detects state changes (e.g. new choice in in `st.selectbox`)
...
## Saving across re-runs {style="font-size: 0.8em"}
...
* But caching is more commonly used but less transparent -- will explain this in next section
* *Note: there are also subtle difference in uses between session states and caching that we won't cover in depth in this class.*

## Session state {style="font-size: 0.8em"}
...
## Session state demo overview {style="font-size: 0.8em"}
...
## Session state demo review code {style="font-size: 0.8em"}
* Lines 9-10: users selects line to cut and percentage cut 
* Lines 12-16: record number of calculations done and define key for calculations
* Lines 19-23: If calculation has not been run before, run it and save in `st.session_state`. Add to counter.
...
## Decorators {style="font-size: 0.8em"}
* A second way to save values across states is to use a `@st.cache_data` **decorator**
* Before we describe `@st.cache_data`, we'll cover decorators in general
...
## Decorator example 1: timing pandas data processing {style="font-size: 0.8em"}

* Lines 3-11: define `@timer` decorator: times function and prints time
* Lines 13-15: apply `@timer` to `summarize(df)`
...
## Decorator example 2: ensure CRS for GeoPandas joins  {style="font-size: 0.8em"}

* Line 5: need to define extra `decorator()` layer to capture the decorator's argument ("EPSG:4326")
...
## `@st.cache_data` decorator {style="font-size: 0.8em"}
...
* `@st.cache_data` saves the result of the function the *first time* it runs
    * If first time run: `@st.cache_data` runs the function and saves result to memory
    * If run before: `@st.cache_data` returns the previously saved result instead of re-running
* `@st.cache_data` trades off memory for speed

## How cacheing works {style="font-size: 0.8em"}
```{python}
#| eval: false
@st.cache_data
...
## Automate toy example via caching

`6/session_state_cache.qmd`:
...
## Summary: decorators and `@st.cache_data` {style="font-size: 0.7em"}
...
## How should you use `streamlit` for the final project (and in life)?  {style="font-size: 0.7em"}
...
* This interactivity should be *meaningful*: should approximate the "exploration mode" loop we outlined last lecture.
    * Example: user selection changes input data for a plot or parameters to a calculation 
    * Counterexample: your dashboard allows user to switch between 2 pre-made static plots
...
What if a job you apply for requires a specific dashboarding software? Spend a few hours teaching it to yourself and then put it on your resume (do **not** just put it on your resume). Or put dashboards in general and learn the skill for the interview.

## The most famous dashboard ever  {style="font-size: 0.8em"}
New York City Police Department's "CompStat" in the 1990's ([link](https://compstat.nypdonline.org/)). Why is it such a perfect application of a dashboard? (Source: *Classic book: Bob Behn's [The PerformanceStat Potential](https://www.brookings.edu/books/the-performancestat-potential/)*)
